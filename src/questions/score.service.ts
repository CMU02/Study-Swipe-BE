...
export class ScoreService {
  ...
  scoreFromBlocks(blocks: IncomingAnswerBlock[]): ScoreResponse {
    ...
    for (const block of blocks) {
      ...
      for (const q of qs) {
        ...
        if (!Number.isFinite(no))
          throw new BadRequestException(`no가 유효하지 않습니다: ${no}`);

        if (!['기초', '경험', '응용'].includes(level))
          throw new BadRequestException(
            `level은 기초/경험/응용 중 하나여야 합니다: no=${no}, level=${level}`,
          );

        if (!Number.isFinite(value) || value < 1 || value > 5)
          throw new BadRequestException(
            `value는 1~5 사이여야 합니다: no=${no}, value=${value}`,
          );

        if (seenNo.has(no))
          throw new BadRequestException(`중복 응답: no=${no}`);
        seenNo.add(no);

        // 가중치 적용
        const w = WEIGHT[level];
        const g = perTagAgg.get(tag) ?? {
          sum: 0,
          wsum: 0,
          wtot: 0,
          cnt: 0,
    ...
    // 전체 집계 및 등급
    if (globalCnt === 0)
      throw new BadRequestException('채점할 문항이 없습니다.');
